...
const selectQuotes = async (req, res) => {
    ...
    try {
        ...
        const response = await pool.query('select id_reservacion_usuario,documento_reservacion,nombres,primer_apellido,segundo_apellido,fecha, hora,nombre_reservacion, tipo_de_estado from reservacion,reservacion_usuario, usuarios, estado where id_reservacion_usuario=id_reservacion and documento_reservacion=documento and state = id_tipo');
...
const selectViewQuotes = async (req, res) => {
    ...
    try {
        ...
        const response = await pool.query('select id_reservacion_usuario,documento_reservacion,nombres,primer_apellido,segundo_apellido,numero_telefono,fecha,hora,nombre_reservacion from reservacion,reservacion_usuario, usuarios, telefono where id_reservacion_usuario=id_reservacion and documento_reservacion=documento and documento_usuario=documento and fecha = $1 order by hora asc',[fecha]);
...
const selectOneQuotes = async (req, res) => {
    ...
    try {
        ...
        const response = await pool.query('select id_reservacion_usuario,documento_reservacion,nombres,primer_apellido,segundo_apellido,numero_telefono,fecha, hora,nombre_reservacion, correo, tipo_de_estado from reservacion,reservacion_usuario, usuarios, telefono, estado where id_reservacion_usuario=id_reservacion and documento_reservacion=documento and state = id_tipo and documento_usuario=documento and documento = $1 and fecha = $2',[documento,fecha]);
...
const searchStateQuotes = async (req, res) => {
    ...
    try {
        const { documento } = req.body;
        const response = await pool.query("SELECT nombre_reservacion, concat( extract( day from fecha) ||' '||'de'||' '|| to_char(fecha,'TMMonth')||' '||'del'||' '||extract(year from fecha)) as date,fecha, concat(extract( hour from hora) ||':'|| extract (minute from hora)) as hora,tipo_de_estado, id_tipo FROM reservacion_usuario, usuarios, reservacion, estado where documento_reservacion=documento and id_reservacion_usuario=id_reservacion and state=id_tipo and documento= $1 order by reservacion_usuario.fecha desc limit 1;",[documento]);
...
const CreateSolicitadAssessment  = async (req, res) => {
    ...
    try {
        ...
        const response = await pool.query('insert into reservacion_usuario ( id_reservacion_usuario, documento_reservacion, fecha, state, hora ) values ($1,$2,current_date,$3,current_time)',[id_reservacion_usuario, documento_reservacion, state]);

        if(response.error){
            res.status(401).json(response.error);
        }else{
            res.status(200).json('Solicitad creada con exito');
        }
...
const selectAllQuotesOneUser = async (req, res) => {
    ...
    try {
        const { documento } = req.body;
        const response = await pool.query("SELECT nombre_reservacion, concat( extract( day from fecha)||' '||'de'||' '|| to_char(fecha,'TMMonth')||' '||'del'||' '||extract(year from fecha)) as date,fecha, concat(extract( hour from hora) ||':'|| extract (minute from hora)) as hora,tipo_de_estado FROM reservacion_usuario, usuarios, reservacion, estado where documento_reservacion=documento and id_reservacion_usuario=id_reservacion and state=id_tipo and documento= $1 and not id_tipo=1",[documento]);
