...
const selectOnePlan = async (req, res) => {
    ...
    try{
        ...
        const response = await pool.query('SELECT id_suscripcion, titulo_suscripcion, descripcion, precio FROM suscripcion where id_suscripcion = $1', [id_suscripcion]);
...
const createPlans = async (req, res) => {
    ...
    try {
        const { titulo_suscripcion, descripcion, precio } = req.body;
        const response = await pool.query('insert into suscripcion (titulo_suscripcion, descripcion, precio) values ($1,$2,$3)',[titulo_suscripcion, descripcion, precio]);
