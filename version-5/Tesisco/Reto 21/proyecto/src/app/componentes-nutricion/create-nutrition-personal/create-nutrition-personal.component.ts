...
@Component({
  selector: 'app-create-nutrition-personal',
  templateUrl: './create-nutrition-personal.component.html',
  styleUrls: ['./create-nutrition-personal.component.css']
})
export class CreateNutritionPersonalComponent implements OnInit {
  ...
  constructor(
    ...
    const temp = localStorage.getItem('nutrition');
    this.textToConvert = temp;
    this.decrypt();
  ...
  cancellProcess(){
    ...
    swalWithBootstrapButtons.fire({
      ...
      if (result.isConfirmed) {
        ...
        const id_nutricional = this.id_nutricional;
        const documento_plan = this.documento_plan;
        let form = {id_nutricional,documento_plan};
    
        this.service.cancellPocessPersonal(form).subscribe(res => {
    
          if(res == 'Proceso personal cancelado con exito'){
